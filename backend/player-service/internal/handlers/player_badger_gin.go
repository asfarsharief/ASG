...
// PlayerHandlerBadgerGin handles HTTP requests for player operations using BadgerDB and Gin
type PlayerHandlerBadgerGin struct {
	repo *database.PlayerRepositoryBadger
}

// NewPlayerHandlerBadgerGin creates a new player handler
func NewPlayerHandlerBadgerGin() *PlayerHandlerBadgerGin {
	return &PlayerHandlerBadgerGin{
		repo: database.NewPlayerRepositoryBadger(),
	}
}

// GetPlayers handles GET /api/v1/players
func (h *PlayerHandlerBadgerGin) GetPlayers(c *gin.Context) {
...
func (h *PlayerHandlerBadgerGin) GetPlayer(c *gin.Context) {
	id := c.Param("id")
	if id == "" {
...
func (h *PlayerHandlerBadgerGin) CreatePlayer(c *gin.Context) {
...
func (h *PlayerHandlerBadgerGin) UpdatePlayer(c *gin.Context) {
	id := c.Param("id")
	if id == "" {
		h.sendErrorResponse(c, http.StatusBadRequest, "Player ID is required", nil)
		return
...
func (h *PlayerHandlerBadgerGin) DeletePlayer(c *gin.Context) {
	id := c.Param("id")
	if id == "" {
		h.sendErrorResponse(c, http.StatusBadRequest, "Player ID is required", nil)
		return
...
func (h *PlayerHandlerBadgerGin) sendErrorResponse(c *gin.Context, statusCode int, message string, err error) {
	errorMsg := message
	if err != nil {
